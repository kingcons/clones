...
**** TODO Enable the PPU to trigger NMI
...
**** TODO Add a render object that tracks the current scanline and accepts an NMI callback
**** TODO Add an on-frame callback to render and implement STEP-FRAME
***** Initially, don't really render. Just have an arbitrary byte array that is used in on-frame.
*** WAIT Explore Raylib
